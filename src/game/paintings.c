...
f32 painting_mario_y(struct Painting *painting) {
    ...
    if (mYOffset < 0.0) {
        ...
        mYOffset = painting->size; // If Mario is past the right side, return the right side
    }

    if (lYOffset < 0.0) {
        lYOffset = 0.0; // If Mario is past the left side, return the left side
    } else if (lYOffset > painting->size) {
        lYOffset = painting->size; // If Mario is past the right side, return the right side
    }

    return mYOffset * (mYOffset < lYOffset) + lYOffset * (mYOffset >= lYOffset);
}
...
f32 painting_mario_z(struct Painting *painting) {
    ...
    if (mZOffset < 0.0) {
        ...
        mZOffset = painting->size; // If Mario is past the right side, return the right side
    }

    if (lZOffset < 0.0) {
        lZOffset = 0.0; // If Mario is past the left side, return the left side
    } else if (lZOffset > painting->size) {
        lZOffset = painting->size; // If Mario is past the right side, return the right side
    }

    return mZOffset * (mZOffset < lZOffset) + lZOffset * (mZOffset >= lZOffset);
}
...
f32 painting_mario_x(struct Painting *painting) {
    ...
    if (mXOffset < 0.0) {
        ...
        mXOffset = painting->size; // If Mario is past the right side, return the right side
    }

    if (lXOffset < 0.0) {
        lXOffset = 0.0; // If Mario is past the left side, return the left side
    } else if (lXOffset > painting->size) {
        lXOffset = painting->size; // If Mario is past the right side, return the right side
    }

    return mXOffset * (mXOffset < lXOffset) + lXOffset * (mXOffset >= lXOffset);
}
